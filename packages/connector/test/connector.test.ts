...
describe('@connector', (): void => {
  ...
  test('createJob is equivalent to createJobs when given a single job', async () => {
    const createJobOutput: CreateJobResponse = (await connector.createJob(jobData)) as CreateJobResponse
    const createJobsOutput: CreateJobResponse[] = (await connector.createJobs([jobData])) as CreateJobResponse[]
