...
export default async function request( apiOptions : APIOptions ) : Promise<any> {
    ...
    let instance : any = createInstance(oauthOptions)
    const options : object = Object.assign({}, apiOptions.additionalOptions, { method : apiOptions.method, data : apiOptions.data, params : apiOptions.params, url : apiOptions.url })
    let response : object = await instance(options)
    return response
}